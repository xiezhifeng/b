...
AJS.Editor.JiraConnector.Panel.Search.prototype = AJS.$.extend(
        AJS.Editor.JiraConnector.Panel.Search.prototype,
        AJS.Editor.JiraConnector.Panel.prototype);
AJS.Editor.JiraConnector.Panel.Search.prototype = AJS.$
        .extend(
                AJS.Editor.JiraConnector.Panel.Search.prototype,
                {
                    ...
                    init : function(panel) {
                        ...
                        var clearPanel = function() {
                            container.children(":not(div.jira-search-form)")
                                    .remove();
                        };
                        ...
                        var doSearch = function(searchStr, serverName) {
                            ...
                            if (this.currentXhr
                                    && this.currentXhr.readyState != 4) {
                                return;
                            }
                            var queryTxt = searchStr
                                    || $('input', container).val();

                            // analytics stuff
                            var type = AJS.Editor.JiraConnector.JQL
                                    .checkQueryType(queryTxt);
                            if (type) {
                            ...
                            // url/url xml
                            if (AJS.Editor.JiraConnector.JQL
                                    .isIssueUrlOrXmlUrl(queryTxt)) {
                                var url = decodeURIComponent(queryTxt);
                                var jiraParams = AJS.Editor.JiraConnector.JQL
                                        .getJqlAndServerIndexFromUrl(
                                                url,
                                                AJS.Editor.JiraConnector.servers);
                                if (processJiraParams(jiraParams)) {
                                    performQuery(jiraParams["jqlQuery"], false,
                                            null);
                                }
                            } else {
                        ...
                        var showNoServerMessage = function(isAdmin) {
                            var message = Confluence.Templates.ConfluenceJiraPlugin
                                    .showMessageNoServer({
                                        'isAdministrator' : isAdmin,
                                        'contentPath' : Confluence
                                                .getContextPath()
                                    })
                            thiz.noServerMsg(container, message);

                            // bind click for call refresh applink select when
                            // user click on open applink config
                        ...
                        // add tipsy tooltip
                        var $optionsPanel = AJS
                                .$('.jql-display-opts-open.disabled');
                        var tipsyOptions = {
                            live : true,
                            title : function() {
                                return AJS.$('.jql-display-opts-open.disabled')
                                        .data('title')
                            },
                    ...
                    addSearchForm : function() {
                        ...
                        var searchFormSoy = Confluence.Templates.ConfluenceJiraPlugin
                                .searchForm({
                                    'isMultiServer' : isMultiServer
                                });
                        var searchForm = AJS.$(searchFormSoy).appendTo(
                                thiz.container);

                        if (servers.length > 1) {
                            var serverSelect = $(
                                    '<select class="select" tabindex="0"></select>')
                                    .insertAfter('div.search-input', searchForm);
                            thiz.applinkServerSelect(serverSelect,
                                    thiz.authCheck);
                        }
                        ...
                        thiz.setActionOnEnter($('input', thiz.container),
                                thiz.doSearch);
                    },
                    ...
                    validate : function() {
                        ...
                        var issueResult = AJS.$(
                                'input:checkbox[name=jira-issue]', container);

                        if (issueResult.length) {
                            var selectedIssueCount = AJS.$(
                                    'input:checkbox[name=jira-issue]:checked',
                                    container).length;
                    ...
                    /*
                     * This function is used for splitting a column string to
                     * array, ex: Custom Columns, key. Note: If a column include
                     * space in need to be put into quotes. Ex: "word1, word2".
                     */
                    parseColumnsStringToArray : function(columnsString) {
                        ...
                        for ( var i = 0; i < columnsString.length; i++) {
                            ...
                            if (currentChar === "\"" || currentChar === "'") {
                                if (currentQuote == null) {
                                    ...
                                    // here we see the closed quote character,
                                    // so we put the column
                        ...
                        // add the last column if it was not pushed into array
                        // in the loop
                        pushColumnToArray(column, result);
                        return result;
                    },
                    ...
                    getMacroParamsFromUserInput : function() {
                        ...
                        var isCount = ((AJS.$(
                                'input:radio[name=insert-advanced]:checked')
                                .val() == "insert-count") ? true : false);
                        var container = this.container;
                        ...
                        if (selectedIssueKeys.length == 1) {
                            ...
                            macroInputParams['key'] = selectedIssueKeys
                                    .toString();
                        }
                    ...
                    insertLink : function(_searchPanel) {
                        var searchPanel;
                        if (_searchPanel
                                && typeof _searchPanel.insertIssueLinkWithParams === 'function') {
                            searchPanel = _searchPanel;
                        } else {
                            searchPanel = this;
                        }
                        var macroInputParams = searchPanel
                                .getMacroParamsFromUserInput();
                    ...
                    selectHandler : function() {
                        ...
                        if (selectedRow.length) {
                            selectedRow.unbind('keydown.space').bind(
                                    'keydown.space',
                                    function(e) {
                                        if (e.which == 32 || e.keyCode == 32) {
                                            var inpChk = selectedRow
                                                    .find('[type=checkbox]');
                    ...
                    updateTotalIssuesDisplay : function(totalIssues) {
                        var jiraIssuesLink = this.selectedServer.url
                    ...
                    prepareColumnInput : function(selectedColumnString) {
                        ...
                        var initColumnInputField = function(data) {
                            ...
                            if (columnInputField.hasClass("chzn-done")) {
                                ...
                                // TODO: The Chosen plugin cannot support 100%
                                // width as it should.
                                columnInputField
                                        .chosen({
                                            "selected_values_in_order" : selectedColumnValues,
                    ...
                    // bind event for displayOptionPanel
                    bindEventToDisplayOptionPanel : function() {
                        ...
                        optDisplayRadios.change(function() {
                            if (optTotalRadio.prop('checked')) {
                                AJS.$("#jiraIssueColumnSelector").attr(
                                        'disabled', true).trigger(
                                        "liszt:updated");
                            } else {
                                AJS.$("#jiraIssueColumnSelector").removeAttr(
                                        'disabled').trigger("liszt:updated");
                            }
                        });

                        columnsDisplayInput = AJS.$(
                                'input:text[name=columns-display]').change(
                                function() {
                                    thiz.customizedColumn = columnsDisplayInput
                                            .val();
                                });
                        ...
                        ticketCheckboxes
                                .change(function() {
                                    thiz.validate();
                                    var ticketUncheckedLength = AJS
                                            .$('#my-jira-search input:checkbox[name=jira-issue]:not(:checked)').length;
                                    if (ticketUncheckedLength > 0) {
                                        ticketCheckboxAll.removeAttr('checked');
                                    } else {
                                        ticketCheckboxAll.prop('checked',
                                                'checked');
                                    }
                                });
                        thiz.validate();
                    ...
                    changeInsertOptionStatus : function(selectedIssueCount) {
                        ...
                        if (selectedIssueCount > 1) {
                            ...
                            AJS.$('.jql-display-opts-open').removeClass(
                                    "disabled");
                        } else {
                            ...
                            AJS.$('.jql-display-opts-open')
                                    .addClass("disabled");
                        }
                    }
                });
AJS.Editor.JiraConnector.Panels
        .push(new AJS.Editor.JiraConnector.Panel.Search());
