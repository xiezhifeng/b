(function($, _){
    ...
    function load() {
        var jiraData = JSON.parse($('#jiraData').val());
        var issues = _.filter(jiraData.issues, function(issue) {
        ...
        releaseData.nodeDataArray.push({'key':'0', text:'Confluence Inline Comment', 'loc':'0 0'});
        var issuesCount = _.size(issues);
            nodeCount = 0,
            nodeLeng = 75,
            stepLeng = 50,
            upperLimit = (issuesCount / 2) * stepLeng,
            lowerLimit = -upperLimit
            upperStep = 0,
            lowerStep = 0;
        var colorSet = ['skyblue', 'darkseagreen', 'palevioletred'];

        _.each(issues, function(issue) {
            nodeCount++;
            releaseData.nodeDataArray.push({
                'key': nodeCount,
                'parent': nodeCount - 1,
                'text': '',
                'brush': 'coral',
                'dir': 'left',
                'loc': '-' + nodeLeng * nodeCount + ' 0'
            });
            releaseData.nodeDataArray.push({
                'key': 10000 + nodeCount,
                'parent': nodeCount,
                'text': issue.fields.summary,
                'brush': colorSet[nodeCount % 3],
                'dir': 'left',
                'loc': '-' + nodeLeng * (nodeCount + 2) + ((nodeCount % 2 == 0) ? ' ' + (lowerLimit + stepLeng * (lowerStep++)) : ' ' + (upperLimit - stepLeng * (upperStep++)))
            });
        });

        myDiagram.model = go.Model.fromJson(JSON.stringify(releaseData));
    }
