...
public class JiraIssuesMacro extends BaseMacro implements Macro, EditorImagePlaceholder, ResourceAware
{
     ...
     */
    public JiraIssuesMacro(final I18NBeanFactory i18NBeanFactory, final JiraIssuesManager jiraIssuesManager, final SettingsManager settingsManager,
            final JiraIssuesColumnManager jiraIssuesColumnManager, final TrustedApplicationConfig trustedApplicationConfig,
            final PermissionManager permissionManager, final ApplicationLinkResolver applicationLinkResolver,
            final JiraIssuesDateFormatter jiraIssuesDateFormatter, final MacroMarshallingFactory macroMarshallingFactory,
            final JiraCacheManager jiraCacheManager, final ImagePlaceHolderHelper imagePlaceHolderHelper, final FormatSettingsManager formatSettingsManager,
            final JiraIssueSortingManager jiraIssueSortingManager, final JiraExceptionHelper jiraExceptionHelper, final LocaleManager localeManager,
    ...
    String getText(final String i18n)
    {
        return getI18NBean().getText(i18n);
    }

    String getText(final String i18n, final List substitutions)
    {
    ...
    {
        final String tokenTypeString = (String) parameters.get(TOKEN_TYPE_PARAM);
    ...
    @Override
    public ImagePlaceholder getImagePlaceholder(final Map<String, String> parameters, final ConversionContext conversionContext)
    {
        try
        {
            final JiraRequestData jiraRequestData = parseRequestData(parameters);
    ...
    protected JiraRequestData parseRequestData(final Map<String, String> params) throws MacroExecutionException {
        ...
        final String requestData = getPrimaryParam(params);
        if (requestData.startsWith("http"))
        {
            return createJiraRequestData(requestData, Type.URL);
        }

        final Matcher keyMatcher = JiraJqlHelper.ISSUE_KEY_PATTERN.matcher(requestData);
    ...
    private JiraRequestData createJiraRequestData(String requestData, final Type requestType) throws MacroExecutionException
    {
        if (requestType == Type.KEY && requestData.indexOf(',') != -1)
        {
            final String jql = "issuekey in (" + requestData + ")";
    ...
    protected void createContextMapFromParams(final Map<String, String> params, final Map<String, Object> contextMap,
            final String requestData, final Type requestType, final ApplicationLink applink,
            final boolean staticMode, final boolean isMobile, final Map<String,JiraColumnInfo> jiraColumns, final ConversionContext conversionContext) throws MacroExecutionException
    {

        final List<String> columnNames = JiraIssueSortableHelper.getColumnNames(JiraUtil.getParamValue(params, COLUMNS, JiraUtil.PARAM_POSITION_1));
        final List<JiraColumnInfo> columns = jiraIssuesColumnManager.getColumnInfo(params, jiraColumns, applink);
        contextMap.put(COLUMNS, columns);
        final String cacheParameter = JiraUtil.getParamValue(params, CACHE, JiraUtil.PARAM_POSITION_2);
        ...
        final String heightStr = JiraUtil.getParamValue(params, HEIGHT, JiraUtil.PARAM_POSITION_6);
        if (!StringUtils.isEmpty(heightStr) && StringUtils.isNumeric(heightStr))
        {
            contextMap.put(HEIGHT, heightStr);
        }

        final String showSummaryParam = JiraUtil.getParamValue(params, SHOW_SUMMARY, JiraUtil.SUMMARY_PARAM_POSITION);
        ...
        final boolean showTrustWarnings = Boolean.valueOf(forceTrustWarningsStr)
        ...
        // display a warning
        final boolean isAdministrator = permissionManager.hasPermission(
        ...
        if (staticMode)
        {
            final String maximumIssuesStr = StringUtils.defaultString(params.get(MAXIMUM_ISSUES), String.valueOf(JiraUtil.DEFAULT_NUMBER_OF_ISSUES));
        ...
        final String baseurl = params.get(BASE_URL);

        final String clickableUrl = getClickableUrl(requestData, requestType, applink, baseurl);
        ...
        final JiraIssuesType issuesType = JiraUtil.getJiraIssuesType(params, requestType, requestData);
        ...
        final boolean userAuthenticated = AuthenticatedUserThreadLocal.get() != null;
        boolean useCache;
        ...
        {
            final int refreshId = getNextRefreshId();

            contextMap.put("refreshId", refreshId);
            final MacroDefinition macroDefinition = new MacroDefinition("jira", new RichTextMacroBody(""), null, params);
            try
            {
                final Streamable out = macroMarshallingFactory.getStorageMarshaller().marshal(macroDefinition, conversionContext);
                final StringWriter writer = new StringWriter();
                out.writeTo(writer);
                contextMap.put("wikiMarkup", writer.toString());
            }
            catch (final XhtmlException e)
            {
            ...
            final String contentId = conversionContext.getEntity() != null ? conversionContext.getEntity().getIdAsString() : "-1";
            contextMap.put("contentId", contentId);

        }
    }

    private void setKeyInContextMap(final String requestData, final Type requestType, final Map<String, Object> contextMap)
    {
    ...
    private void populateContextMapForStaticSingleIssue(
            final Map<String, Object> contextMap, final String url,
            final ApplicationLink applicationLink, final boolean forceAnonymous, final boolean useCache, final ConversionContext conversionContext)
                    throws MacroExecutionException
    {
    ...
    private void populateContextMapForStaticSingleIssueAnonymous(
            final Map<String, Object> contextMap, final String url,
            final ApplicationLink applink, final boolean forceAnonymous, final boolean useCache, final ConversionContext conversionContext)
                    throws MacroExecutionException
    {
        JiraIssuesManager.Channel channel;
        try
    ...
    private void setupContextMapForStaticSingleIssue(final Map<String, Object> contextMap, final Element issue, final ApplicationLink applicationLink)
    {
        final Element resolution = issue.getChild("resolution");
        final Element status = issue.getChild("status");
        ...
        final String key = issue.getChild(KEY).getValue();
        contextMap.put(KEY, key);
        ...
        final Element statusCategory = issue.getChild("statusCategory");
        if (null != statusCategory)
        {
            final String colorName = statusCategory.getAttribute("colorName").getValue();
            final String keyName = statusCategory.getAttribute("key").getValue();
    ...
    private String getXmlUrl(final int maximumIssues, final String requestData, final Type requestType,
            final ApplicationLink applicationLink) throws MacroExecutionException {
        final StringBuilder stringBuilder = new StringBuilder(JiraUtil.normalizeUrl(applicationLink.getRpcUrl()));
        ...
        switch (requestType)
        {
            case URL:
                ...
                {
                    final String jql = JiraJqlHelper.getJQLFromFilter(applicationLink, requestData, jiraIssuesManager, getI18NBean());
                ...
                else
                {
                    ...
                    final String jql = JiraJqlHelper.getJQLFromJQLURL(requestData);
                    if (jql != null)
                    {
                        stringBuilder.append(JiraUtil.utf8Encode(jql));
                        return stringBuilder.toString();
                    ...
                    {
                        final String key = JiraJqlHelper.getKeyFromURL(requestData);
    ...
    private String buildKeyJiraUrl(final String key, final ApplicationLink applicationLink)
    {
        final String encodedQuery = JiraUtil.utf8Encode("key in (" + key + ")");
    ...
    private String getClickableUrl(final String requestData, final Type requestType,
            final ApplicationLink applicationLink, final String baseurl)
    ...
    private String appendSourceParam(final String clickableUrl)
    {
        final String operator = clickableUrl.contains("?") ? "&" : "?";
     ...
     */
    private void populateContextMapForStaticTable(final Map<String, Object> contextMap, final List<String> columnNames, final String url,
            final ApplicationLink appLink, final boolean forceAnonymous, final boolean useCache, final ConversionContext conversionContext) throws MacroExecutionException
    {
        final boolean clearCache = getBooleanProperty(conversionContext.getProperty(DefaultJiraCacheManager.PARAM_CLEAR_CACHE));
        try
        {
            ...
            final JiraIssuesManager.Channel channel = jiraIssuesManager.retrieveXMLAsChannel(url, columnNames, appLink,
                    forceAnonymous, useCache);
    ...
    private void populateContextMapForStaticTableByAnonymous(final Map<String, Object> contextMap, final List<String> columnNames,
            final String url, final ApplicationLink appLink, final boolean forceAnonymous, final boolean useCache)
                    throws MacroExecutionException
    {
        try
        {
            final JiraIssuesManager.Channel channel = jiraIssuesManager.retrieveXMLAsChannelByAnonymous(url, columnNames,
    ...
    private void setupContextMapForStaticTable(final Map<String, Object> contextMap, final JiraIssuesManager.Channel channel, final ApplicationLink appLink)
    {
        ...
        else
        {
            try
            {
                final URL sourceUrl = new URL(channel.getSourceUrl());
                final String jiraServerUrl = sourceUrl.getProtocol() + "://" + sourceUrl.getAuthority();
        ...
        final Locale locale = localeManager.getLocale(AuthenticatedUserThreadLocal.get());
        contextMap.put("dateFormat", new SimpleDateFormat(formatSettingsManager.getDateFormat(), locale));
    }

    private void populateContextMapForStaticCountIssues(final Map<String, Object> contextMap, final List<String> columnNames,
            final String url, final ApplicationLink appLink, final boolean forceAnonymous, final boolean useCache, final ConversionContext conversionContext) throws MacroExecutionException
    {
        try
        {
            final JiraIssuesManager.Channel channel = jiraIssuesManager.retrieveXMLAsChannel(url, columnNames, appLink, forceAnonymous, useCache);
            final Element element = channel.getChannelElement();
            final Element totalItemsElement = element.getChild("issue");
            final String count = totalItemsElement != null ? totalItemsElement.getAttributeValue("total") : "" + element.getChildren("item").size();
    ...
    private String getCountIssuesWithAnonymous(final String url, final List<String> columnNames, final ApplicationLink appLink, final boolean forceAnonymous, final boolean useCache) throws MacroExecutionException {
        try {
            final JiraIssuesManager.Channel channel = jiraIssuesManager.retrieveXMLAsChannelByAnonymous(url, columnNames, appLink, forceAnonymous, useCache);
            final Element element = channel.getChannelElement();
            final Element totalItemsElement = element.getChild("issue");
    ...
    private void populateContextMapForDynamicTable(
            final Map<String, String> params, final Map<String, Object> contextMap, final List<JiraColumnInfo> columns,
            final boolean useCache, final String url, final ApplicationLink applink, final boolean forceAnonymous) throws MacroExecutionException
    {
        ...
        final String startOn = getStartOnParam(params.get("startOn"), urlBuffer);
    ...
    private String getStartOnParam(final String startOn, final StringBuffer urlParam)
    {
        final String pagerStart = filterOutParam(urlParam,"pager/start=");
    ...
    private String getSortOrderParam(final StringBuffer urlBuffer)
    {
        final String sortOrder = filterOutParam(urlBuffer, "sorter/order=");
    ...
    private String getSortFieldParam(final StringBuffer urlBuffer)
    {
        final String sortField = filterOutParam(urlBuffer, "sorter/field=");
    ...
    // old macros
    protected String getPrimaryParam(final Map<String, String> params) throws MacroExecutionException {
    ...
    private boolean shouldRenderInHtml(final String renderModeParamValue, final ConversionContext conversionContext) {
    ...
    protected int getResultsPerPageParam(final StringBuffer urlParam)
            throws MacroExecutionException
    {
        final String tempMaxParam = filterOutParam(urlParam, "tempMax=");
        if (StringUtils.isNotEmpty(tempMaxParam))
        {
            final int tempMax = Integer.parseInt(tempMaxParam);
            if (tempMax <= 0)
            {
    ...
    protected static String filterOutParam(final StringBuffer baseUrl,
            final String filter) {
        final int tempMaxParamLocation = baseUrl.indexOf(filter);
        if (tempMaxParamLocation != -1)
        {
            String value;
            final int nextParam = baseUrl.indexOf("&", tempMaxParamLocation);
    ...
    public String rebaseUrl(final String clickableUrl, final String baseUrl) {
    ...
    protected static String makeClickableUrl(final String url) {
        final StringBuffer link = new StringBuffer(url);
    ...
    private String buildRetrieverUrl(final Collection<JiraColumnInfo> columns,
            final String url, final ApplicationLink applicationLink, final boolean forceAnonymous)
    {
        final String baseUrl = settingsManager.getGlobalSettings().getBaseUrl();
        final StringBuilder retrieverUrl = new StringBuilder(baseUrl);
        ...
        for (final JiraColumnInfo columnInfo : columns)
        {
    ...
    @Override
    public String execute(final Map<String, String> parameters, final String body, final ConversionContext conversionContext) throws MacroExecutionException
    {
        final JiraRequestData jiraRequestData = parseRequestData(parameters);
        ...
        catch (final TypeNotInstalledException tne)
        {
        ...
        final Map<String, JiraColumnInfo> jiraColumns = jiraIssuesColumnManager.getColumnsInfoFromJira(applink);
        ...
        final JiraIssuesType issuesType = JiraUtil.getJiraIssuesType(parameters, requestType, requestData);
        ...
        final boolean staticMode = shouldRenderInHtml(parameters.get(RENDER_MODE_PARAM), conversionContext);
        final boolean isMobile = MOBILE.equals(conversionContext.getOutputDeviceType());
    ...
    private Locale getUserLocale(final String language)
    {
    ...
    private boolean getBooleanProperty(final Object value)
    {
    ...
    private void setRenderMode(final Map<String, Object> contextMap, final String outputType)
    {
    ...
    public String renderSingleJiraIssue(final Map<String, String> parameters, final ConversionContext conversionContext, final Element issue, final String serverUrl) throws Exception {
        final Map<String, Object> contextMap = MacroUtils.defaultVelocityContext();
        final String outputType = conversionContext.getOutputType();
        // added parameters for pdf export
        setRenderMode(contextMap, outputType);

        final String showSummaryParam = JiraUtil.getParamValue(parameters, SHOW_SUMMARY, JiraUtil.SUMMARY_PARAM_POSITION);
        if (StringUtils.isEmpty(showSummaryParam))
        {
            contextMap.put(SHOW_SUMMARY, true);
        }
