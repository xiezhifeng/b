...
public class JiraIssuesPage extends ViewPage
{
    ...
    {
        final int tableRowOffset = 2; //one row for header, and one row is kept by confluence above header with empty data
        waitUntilTrue("JIRA issues table is not visible", issuesTable.timed().isPresent());
        final String xpathSelector = String.format("//table[@class='aui']/tbody/tr[%s]/td[%s]", row + tableRowOffset, column);
